        ...
        if(propertiesUri != null) {
            if ("/login".equals(uri) && "POST".equals(method)) {//查看是否登录请求
                ...
                if (uri.startsWith("/druid")
                    ...
                    Session session = redisService.get(SessionRedisKey.SESSION_KEY, (String) subject.getSession().getId(), Session.class);
...
//                    return super.onPreHandle(request, response, mappedValue);
    ...
    protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
        ...
        try {
            ...
            redisService.set(SessionRedisKey.SESSION_KEY, (String) subject.getSession().getId(), subject.getSession());
